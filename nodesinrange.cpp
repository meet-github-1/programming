...
void getnodeinrange(Node *root,int low,int high,vector<int>&v){
    if(root==NULL){
        return ;
    }
    getnodeinrange(root->left,low,high,v);
    //cout<<root->data<<endl;
    if(root->data>=low && root->data<=high){
        v.push_back(root->data);
    }
    getnodeinrange(root->right,low,high,v);
    return ;
}
vector<int> printNearNodes(Node *root, int low, int high)
{
  // your code goes here  
  vector<int> v;
  getnodeinrange(root,low,high,v);
  return v;
}
